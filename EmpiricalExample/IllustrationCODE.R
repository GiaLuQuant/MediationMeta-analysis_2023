...
library('metaSEM')
library('xlsx')
pridata = read.xlsx('/Users/luzhiming/000我/sysu/自己/主要/mediation-meta-analysis/Gu2015/Analyzied_data.xlsx', sheetIndex = 1)
Pridata = data.frame(pridata)
EffectSize = matrix(NA, 31, 4); colnames(EffectSize)=c('N','rXM','rMY','rXY')
for(i in 4:7){
  for(j in 1:31){
    if(i==4){
    }
...
IE.se = sqrt(0.636^2*vcov(fit0)[3,3] + 0.263^2*4*vcov(fit0)[1,1] + 2*0.318*0.263*vcov(fit0)[1,3])

1-pnorm(0.318*0.263/IE.se)
