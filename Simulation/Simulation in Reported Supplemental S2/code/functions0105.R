...
dg.ps <- function(StCoef, K, Tau, Nbar, Rho, postVar, beta1, Missing, MissingR, p=.5){
...
# study 1
ma <- function(StCoef, K, Tau, Nbar, Rho, nrep, postVar, beta1, Missing = FALSE, MissingR = 0){
  RES = list()
  ...
  for (i in 1:nrep){
    dat = try(dg.ps(StCoef, K, Tau, Nbar, Rho, postVar, beta1, Missing, MissingR, p=0.5))
...
# function for deleting negtive definite results
select_res2 <- function(data){
  for(i in 1:nrow(data)){
    if(is.na(data[i,'a.se'])|is.na(data[i,'b.se'])|is.na(data[i,'cp.se'])|is.na(data[i,'beta1.se'])){
...
cal_res <- function(data,a.s,cp.s,b.s,beta1){
  ...
  seM <- apply(data.s[,7:10],2,mean, na.rm=T)
...
# simulation results 
cal_res2 <- function(data,a.s,cp.s,b.s,beta1){
  p=.5
  sigma.X = sqrt(p*(1-p))
  # deleting negative-definite repetitions
  ori = nrow(data)
  data.s <- select_res2(data)
  fin = nrow(data.s)
